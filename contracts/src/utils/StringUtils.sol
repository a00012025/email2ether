...
library StringUtils {
     ...
     * String To Address
     * 
     * Convert a string without 0x prefix to an address
     * 
     * @param addressAsStringWithout0x The address as a string without the 0x prefix
     * @return address
     */

    function stringToAddress(string memory addressAsStringWithout0x) public pure returns (address) {
        bytes memory tmp = bytes(addressAsStringWithout0x);
        uint160 iaddr = 0;
        uint160 b1;
        uint160 b2;
