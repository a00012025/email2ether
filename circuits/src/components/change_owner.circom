...
// regex: \r\nsubject:Change owner to 0x[A-Fa-f0-9]+
template ChangeOwnerRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[0][i].in[0] <== eq[19][i].out;
		multi_or[0][i].in[1] <== eq[21][i].out;
		multi_or[0][i].in[2] <== eq[22][i].out;
		multi_or[0][i].in[3] <== eq[23][i].out;
		multi_or[0][i].in[4] <== eq[24][i].out;
		multi_or[0][i].in[5] <== eq[25][i].out;
		multi_or[0][i].in[6] <== eq[26][i].out;
		multi_or[0][i].in[7] <== eq[27][i].out;
		multi_or[0][i].in[8] <== eq[28][i].out;
		multi_or[0][i].in[9] <== eq[29][i].out;
		multi_or[0][i].in[10] <== eq[30][i].out;
		multi_or[0][i].in[11] <== eq[31][i].out;
		multi_or[0][i].in[12] <== eq[10][i].out;
		multi_or[0][i].in[13] <== eq[32][i].out;
		multi_or[0][i].in[14] <== eq[33][i].out;
		multi_or[0][i].in[15] <== eq[34][i].out;
		multi_or[0][i].in[16] <== eq[12][i].out;
		multi_or[0][i].in[17] <== eq[4][i].out;
		multi_or[0][i].in[18] <== eq[7][i].out;
		multi_or[0][i].in[19] <== eq[35][i].out;
		multi_or[0][i].in[20] <== eq[6][i].out;
		multi_or[0][i].in[21] <== eq[36][i].out;
		and[28][i].b <== multi_or[0][i].out;
		and[29][i] = AND();
		and[29][i].a <== states[i][29];
		and[29][i].b <== multi_or[0][i].out;
		multi_or[1][i] = MultiOR(2);
		multi_or[1][i].in[0] <== and[28][i].out;
		multi_or[1][i].in[1] <== and[29][i].out;
		states[i+1][29] <== multi_or[1][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][29];
	}
	out <== final_state_result.out;
	signal is_consecutive[msg_bytes+1][2];
	is_consecutive[msg_bytes][1] <== 1;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][29] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
	}
	// substrings calculated: [{(28, 29), (29, 29)}]
	signal is_substr0[msg_bytes][3];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		 // the 0-th substring transitions: [(28, 29), (29, 29)]
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][28] * states[i+2][29];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][29] * states[i+2][29];
