...
// regex: Change owner to 0x[a-fA-F0-9]+
template ChangeOwnerRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[0][i].in[12] <== eq[0][i].out;
		multi_or[0][i].in[13] <== eq[24][i].out;
		multi_or[0][i].in[14] <== eq[25][i].out;
		multi_or[0][i].in[15] <== eq[26][i].out;
		multi_or[0][i].in[16] <== eq[2][i].out;
		multi_or[0][i].in[17] <== eq[27][i].out;
		multi_or[0][i].in[18] <== eq[28][i].out;
		multi_or[0][i].in[19] <== eq[29][i].out;
		multi_or[0][i].in[20] <== eq[5][i].out;
		multi_or[0][i].in[21] <== eq[30][i].out;
		...
		states[i+1][19] <== multi_or[1][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][19];
	}
	out <== final_state_result.out;
	signal is_consecutive[msg_bytes+1][2];
	is_consecutive[msg_bytes][1] <== 1;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][19] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
	}
	// substrings calculated: [{(16, 17), (17, 18), (18, 19), (19, 19)}]
	signal is_substr0[msg_bytes][5];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		 // the 0-th substring transitions: [(16, 17), (17, 18), (18, 19), (19, 19)]
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][16] * states[i+2][17];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][17] * states[i+2][18];
		is_substr0[i][3] <== is_substr0[i][2] + states[i+1][18] * states[i+2][19];
		is_substr0[i][4] <== is_substr0[i][3] + states[i+1][19] * states[i+2][19];
