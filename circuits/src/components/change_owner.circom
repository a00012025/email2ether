...
// regex: ((\r\n)|^)subject:Change owner to [A-Fa-f0-9]+
template ChangeOwnerRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[1][i] = MultiOR(22);
		multi_or[1][i].in[0] <== eq[19][i].out;
		multi_or[1][i].in[1] <== eq[20][i].out;
		multi_or[1][i].in[2] <== eq[21][i].out;
		multi_or[1][i].in[3] <== eq[22][i].out;
		multi_or[1][i].in[4] <== eq[23][i].out;
		multi_or[1][i].in[5] <== eq[24][i].out;
		multi_or[1][i].in[6] <== eq[25][i].out;
		multi_or[1][i].in[7] <== eq[26][i].out;
		multi_or[1][i].in[8] <== eq[27][i].out;
		multi_or[1][i].in[9] <== eq[28][i].out;
		multi_or[1][i].in[10] <== eq[29][i].out;
		multi_or[1][i].in[11] <== eq[30][i].out;
		multi_or[1][i].in[12] <== eq[10][i].out;
		multi_or[1][i].in[13] <== eq[31][i].out;
		multi_or[1][i].in[14] <== eq[32][i].out;
		multi_or[1][i].in[15] <== eq[33][i].out;
		multi_or[1][i].in[16] <== eq[12][i].out;
		multi_or[1][i].in[17] <== eq[4][i].out;
		multi_or[1][i].in[18] <== eq[7][i].out;
		multi_or[1][i].in[19] <== eq[34][i].out;
		multi_or[1][i].in[20] <== eq[6][i].out;
		multi_or[1][i].in[21] <== eq[35][i].out;
		and[27][i].b <== multi_or[1][i].out;
		and[28][i] = AND();
		and[28][i].a <== states[i][27];
		and[28][i].b <== multi_or[1][i].out;
		multi_or[2][i] = MultiOR(2);
		multi_or[2][i].in[0] <== and[27][i].out;
		multi_or[2][i].in[1] <== and[28][i].out;
		states[i+1][27] <== multi_or[2][i].out;
	...
	for (var i = 0; i <= num_bytes; i++) {
		final_state_result.in[i] <== states[i][27];
	}
	out <== final_state_result.out;
	signal is_consecutive[msg_bytes+1][2];
	is_consecutive[msg_bytes][1] <== 1;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][27] * (1 - is_consecutive[msg_bytes-i][1]) + is_consecutive[msg_bytes-i][1];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
	}
	// substrings calculated: [{(26, 27), (27, 27)}]
	signal is_substr0[msg_bytes][3];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		is_substr0[i][0] <== 0;
		 // the 0-th substring transitions: [(26, 27), (27, 27)]
		is_substr0[i][1] <== is_substr0[i][0] + states[i+1][26] * states[i+2][27];
		is_substr0[i][2] <== is_substr0[i][1] + states[i+1][27] * states[i+2][27];
